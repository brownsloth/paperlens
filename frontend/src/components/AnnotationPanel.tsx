      ...
      <div className="border-b border-[var(--border)] px-5 py-4">
        ...
        <p className="mt-1 text-xs text-[var(--muted)]">
          {annotations.length > 0
            ...
            : seeded
              ? "No annotations yet — go back to Library and click Annotate"
              : "No annotations yet"}
        </p>
      </div>
      <div ref={listRef} className="flex-1 overflow-y-auto">
        {annotations.length === 0 ? (
          <div className="p-5 text-sm leading-relaxed text-[var(--muted)]">
            <p>This speech is loaded from your seeded library.</p>
            <p className="mt-2">Use <strong>Annotate</strong> in the library to run web search, evidence scoring, and generate notes.</p>
          </div>
        ) : (
